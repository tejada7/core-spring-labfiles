...
class AccountControllerBootTests {
    ...
    void accountDetails() throws Exception {
        ...
        then(accountManager).should().getAccount(0L);
    }
    ...
    @Test
    void accountDetailsFail() throws Exception {
    ...
    @Test
    void should_retrieve_all_accounts() throws Exception {
        // Given
        given(accountManager.getAllAccounts()).willReturn(List.of(
                new Account("1", "account1"),
                new Account("2", "account2"),
                new Account("3", "account3"),
                new Account("4", "account4"),
                new Account("5", "account5")
        ));

        // When
        final var result = mockMvc.perform(get("/accounts"))
                // Then
                .andExpect(status().isOk())
                .andExpect(content().contentType(APPLICATION_JSON))
                .andReturn();

        final var accounts = new ObjectMapper().readValue(result.getResponse().getContentAsByteArray(),
                                                          Account[].class);
        BDDAssertions.then(accounts).hasSize(5);
    }
}

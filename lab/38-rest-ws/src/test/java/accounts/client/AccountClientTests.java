...
public class AccountClientTests {
	...
	void addAndDeleteBeneficiary() {
		...
		final var davidUri = restTemplate.postForLocation(BASE_URL + "/accounts/1/beneficiaries", "David");
		// TODO-14: Retrieve the Beneficiary you just created from the location that was returned
		Beneficiary newBeneficiary = restTemplate.getForObject(davidUri, Beneficiary.class); // Modify this line to use the restTemplate

		assertNotNull(newBeneficiary);
		assertEquals("David", newBeneficiary.getName());

		// TODO-15: Delete the newly created Beneficiary
		restTemplate.delete(davidUri);

		HttpClientErrorException httpClientErrorException = assertThrows(HttpClientErrorException.class, () -> {
			System.out.println("You SHOULD get the exception \"No such beneficiary with name 'David'\" in the server.");

			restTemplate.getForObject(davidUri, Beneficiary.class);
